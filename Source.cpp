...
LRESULT CALLBACK CBTProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	case HCBT_ACTIVATE:
		...
		CreateWindowEx(WS_EX_TRANSPARENT, L"BUTTON", L"CHECK", WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX, 10, 70, 65, 10, (HWND)wParam, (HMENU)CHKID, GetModuleHandle(NULL), NULL);
		{
			HFONT hFont = (HFONT)SendDlgItemMessage((HWND)wParam, IDYES, WM_GETFONT, 0, 0);
			if (hFont)
			{
				SendDlgItemMessage((HWND)wParam, CHKID, WM_SETFONT, (WPARAM)hFont, 0);
			}
		}
		return 0;
	}
...
int CheckBoxMessageBox(HWND hWnd, LPCWSTR lpszMessage, LPCWSTR lpszTitle, UINT uFlags)
{
	hMessageBoxHook = SetWindowsHookEx(WH_CBT, CBTProc, NULL, GetCurrentThreadId());
	return MessageBox(hWnd, lpszMessage, lpszTitle, uFlags);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hButton = CreateWindow(TEXT("BUTTON"), TEXT("メッセージボックス表示"), WS_VISIBLE | WS_CHILD, 0, 0, 0, 0, hWnd, (HMENU)IDOK, ((LPCREATESTRUCT)lParam)->hInstance, 0);
	...
	case WM_COMMAND:
		...
		{
			DWORD dwRetrun = CheckBoxMessageBox(hWnd, L"The quick brown fox jumps over the lazy dog", L"The brown fox", MB_YESNO);
			SetWindowText(hEdit, 0);
			TCHAR szText[1024];
			wsprintf(szText, TEXT("戻り値 = %d"), dwRetrun);
